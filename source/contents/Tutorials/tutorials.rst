...
The HyperCells and HyperBloch packages offer a framwork for efficient modeling 
...
.. card:: Supercells
   ...
   computational group theory. The tight-binding models on these supercells enables systematic
...
.. card:: Coherent sequences
   ...
   sequences, **cohrent sequences**. In this tutorial we showcase how such sequences can be identified.
   Two approaches are presented, which entail the construction of custom functions and **normal subgroup 
   tree graphs** which describes the normal subgroup relations between any pairwise disctinct translation 
   group of a :math:`\{p,q\}`-tessellation of the hyperbolic plane.
...
.. card:: Flat-bands
   ...
   The HyperCells package is equipped with built-in functions to model strongly correlated systems in hyperbolic lattices.
   Among prominent lattices which have emergent flat-bands are **Lieb** and **kagome** lattices.
   In this tutorial we will see how hyperbolic Lieb and kagome lattices are constructed 
   through model graphs in tandem with the application of the supercell method. We will discusss
   their implementation in order to showcase how **custom model graphs** can be constructed.
...
.. card:: Haldane model
   ...
   Haldane models on hyperbolic lattices can be constrcuted by using the HyperCells and HyperBloch packages
   in tandem. The HyperCells package provides the necessary framework to decorate nearest-neighbor models
   with next-nearest-neighbor terms through a minimal modification of the usual workflow. These models can be 
